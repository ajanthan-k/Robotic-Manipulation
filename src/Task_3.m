...
function out_commands = Task_3(task_ID, pen_placement, current_pose)
    ...
    elseif(task_ID == "dropoff")
        ...
        pen_end_position = [pen_placement(1)-((pen_to_floor_dist+pen_length)*cosd(60)), 0, ((pen_to_floor_dist+pen_length)*sind(60)),-stand_angle];
        out_commands = dropoff_pen(pen_end_position, tip_length, stand_angle, current_pose, open_state);
    end
end
...
function out_list = dropoff_pen(pen_end, tip, angle, current_pose, open_state)
    out_list = [];
    hovering_angle = -45;
    cap_entry_leeway = 1;
    push_leeway = 0.5;
    dropoff_clearance = 7;
    working_height = pen_end(3) + dropoff_clearance;
    % Move to working height above pen cap
    out_list(end+1,:) = [current_pose(1), current_pose(2), working_height, hovering_angle];
    out_list(end+1,:) = [pen_end(1)-((tip+2*cap_entry_leeway)*cosd(angle)), pen_end(2), working_height, -angle];
    % Position pen tip
    out_list(end+1,:) = [pen_end(1)-((tip+cap_entry_leeway)*cosd(angle)), pen_end(2), pen_end(3)+((tip+cap_entry_leeway)*sind(angle)), -angle];
    % Push and release
    out_list(end+1,:) = [pen_end(1)+(push_leeway*cosd(angle)), pen_end(2), pen_end(3)+(push_leeway*sind(angle)), -angle];
    out_list(end+1,:) = ["gripper", open_state, 0, 0];
    % Retreat and return to position
    out_list(end+1,:) = [pen_end(1)-((tip+cap_entry_leeway)*cosd(angle)), pen_end(2), pen_end(3)+((tip+cap_entry_leeway)*sind(angle)), -angle];
    out_list(end+1,:) = [pen_end(1)-((tip+2*cap_entry_leeway)*cosd(angle)), pen_end(2), working_height, -angle];
    out_list(end+1,:) = [current_pose];
end