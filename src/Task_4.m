...
for i = 1:numberOfCracks
    movePos([-19, 23, jugHeight+5, 10], port_num, 1, 3);
    setVAProfile(200, 100, port_num);
    movePos([-19, 23, jugHeight, -10], port_num, 1, 3);
    setVAProfile(1000, 200, port_num);
end
% Pull back to open egg halves then raise again
movePos([-15.1, 15.1, jugHeight, 0], port_num, 1, 3);
...
movePos([-13, 14.5, 14, -60], port_num, 1.0, 3);
%setVAProfile(200,100,port_num);
setVAProfile(100,500,port_num); %uncomment prev line once we're sure it's safe
moveArc(-14.5, 14.5, 14, -60, 1.5, 2*pi, -4*pi, 180, port_num, 0.1, 3);
...
movePos([15.8, 17, 10, 0], port_num, 1.0, 3); 
movePos([15.8, 17, 7, 0], port_num, 1.0, 3);
openGripper(2, 1, port_num);
pause(0.5);
...
movePos([0, 20, 3.8, 0], port_num, 1, 3);
...
pause(2);
movePos(flip_motion(1, 1:4), port_num, 1, 3);
movePos([0, 30, 12, -30], port_num, 1, 3);
pause(1)
